...
Acceptance criteria:
  - dispatchItem 6 阶段 progress 事件 + OperationDto.summary（N/M 完成 · 执行中 · 等待中）；CLI plan run/chief run 订阅打印 \r 行；Web 列表/详情展示 summary
  - ENTRY.md.tmpl 追加自我进化引导节；ensureAgentDocsAllowed 幂等放行四文档；prepareRuntime 调用
  - runAgent/runChat/runJob 后 commitSelfEvolution 单文件提交（evolve: 更新 <basename>）；knowledgeWrite 提交（evolve: 更新知识）；均 best-effort 不阻断
  - 规划门脏审计不豁免 agent/*.md（planning 改 ROLE.md → item failed）
  - 全量单测 342 过（+4 self-evolution +3 orchestration）；/code-review 双轴通过；任务完成即 commit（不 push）
