...
| TASK-020 | 记忆系统剩余批次合并（OP2-F + CLI 结构化输出 + OP4-C + OP1 Stage B-E + OP5 A-E）                                                                 | claude-20260803-01 | DONE   | master (TASK-020 commit(s))        | 12 阶段（extensions/backup/paths/agent-schema/process-runner/observability/operation-manager/knowledge/templates/transcript/experience/archival/service-adapter/factory-services/systemd-service/runtime/doctor/application/cli-program/web 与测试 + DECISIONS ADR）                                                                                                                                                                                                                                                                   | TASK-019、用户批准的统一合并计划（12 阶段顺序依赖）                                                                | 2026-08-07 12:20 +0800 |
...
| TASK-027 | Web 编排写面 + 单轮对话（放开 D-022/D-023 只读边界：Todo 创建/加项/派发 + Chief 发起 + 对话标签）                                                | claude-20260803-01 | DONE   | main (TASK-027 commit)             | src/application(factory-application startPlanWithChief+runChat)/src/web/server.ts(5 写路由)/web/src/api.ts(5 方法)/AgentDetailPage(TodoTab 写面+Chief 发起+ChatTab)/styles.css + web-server.test.ts 2 新测 + web-ui.test.tsx 3 新测 + ARCHITECTURE/TESTING/DECISIONS(D-024) + .agent 簿记                                                                                                                                                                                                                                              | 用户批准的 S1+S2 范围（S3 飞书入站单独立项）                                                                       | 2026-08-07 12:20 +0800 |
...
| TASK-028 | CURRENT_STATE.md 自动更新（D-025：系统侧生命周期事件自动更新 + 员工自维护引导/放行 + 单文件自动 git 提交）                                       | claude-20260803-01 | DONE   | main                               | src/core/current-state.ts(新)+git.ts(gitCommitFile)+templates.ts(种子+放行)+templates/*-agent/ENTRY.md.tmpl(引导段)+factory-application.ts(5 事件点+prepareRuntime 补放行) + tests/current-state.test.ts(新)+git/create-agent/runtime/application-management 增测 + ARCHITECTURE/DECISIONS(D-025)/README + .agent 簿记                                                                                                                                                                                                                 | 用户批准的三项决策（仅关键状态 / 引导约定+放行 / 自动提交该文件）                                                  | 2026-08-07 12:20 +0800 |
...
| TASK-031 | 员工自我配置定时任务（D-028：managed_by 标记 + 自动生效 + 文件 YAML 自我进化）                                                                   | claude-20260803-01 | DONE   | main                               | job-schema managed_by+scheduler listTolerant+core/job-reconcile.ts 新+factory-application reconcile 钩子+prepareRuntime 放行 automation/jobs/**+templates ENTRY 定时任务节+automation/jobs/README 种子+web JobConfig managed_by+任务 tab 徽标+styles + tests/job-reconcile.test.ts 新(5)+schemas/self-evolution 增测 + DECISIONS(D-028)/ARCHITECTURE/README/GLOSSARY + .agent 簿记                                                                                                                                                     | 用户批准（TASK-031 计划：managed_by/自动生效/文件 YAML 自我进化）                                                  | 2026-08-07 12:20 +0800 |
| TASK-032 | 描述生成员工 + 自进化拓宽（D-029：移除预设、描述→AI 生成蓝图、commitSelfEvolution 扩展到 skills/workflows/knowledge）                            | claude-20260803-01 | DONE   | main                               | src/core/employee-generator.ts 新+create-agent 去 preset/synthesizeProfile+templates 去 loadPreset+factory-application generateProfile+commitSelfEvolution 拓宽+web server generate 路由+web api+CreateAgentPage 重构+cli --describe/去 --preset+styles + tests/employee-generator 新(5)+迁移+self-evolution 增测+cli-structure+web-ui 增测 + DECISIONS(D-029)/ARCHITECTURE/README/GLOSSARY + .agent 簿记；删除 presets/{user-operations,growth,monetization,engineering}.yaml（保留 cc-switch-allowlist.json）                        | 用户批准（TASK-032 计划：描述生成模型=本地 Claude CLI / 迭代范围=生成+自进化拓宽 / 完全移除预设 / 入口=Web+CLI）   | 2026-08-07 12:20 +0800 |
...
Updated at: 2026-08-07 12:20 +0800
Result: 12 阶段全部提交（阶段1 OP2-F…阶段12 OP5-E PathLayout 收敛，各阶段 commit 见下）。核对 git 历史确认全部在主分支：OP2-F（1ce4002）、CLI 结构化输出（246fdaf）、OP4-C（3378ea5）、OP1 Stage B-E（bf89578/bcab4eb/079db10/74fbba9）、OP5-A/B/C/D/E（f63c735/0dc0b60/98eed02/8b22bef/d3ae75e）。全量 235 单测 + build/lint/prettier 全绿。看板状态由 TASK-046 收尾时核对翻转（原 IN_PROGRESS 为簿记未更新）。
...
```text
Task ID: TASK-029
Title: 派发进度反馈（A+C）+ 员工自我进化（岗位/目标/工作系统/规则 + knowledge 自动提交）
Owner agent: claude-20260803-01
Status: DONE
Branch/worktree: main
Allowed scope: src/core/operation-manager.ts（OperationDto/OperationEvent.summary）、src/application/factory-application.ts（dispatchItem 阶段事件 + dispatchPlan 波形 summary + summaryOf/progressOf + orchestrate onProgress + commitAgentFile/commitSelfEvolution 三处挂载 + knowledgeWrite 提交 + prepareRuntime 放行）、src/cli-program.ts（plan run/chief run \r 进度行）、web/src/api.ts + OperationsDrawer.tsx + styles.css（summary 展示）、src/core/current-state.ts（ensureAgentDocsAllowed 参数化）、templates/*-agent/ENTRY.md.tmpl（自我进化引导节）、tests/orchestration.test.ts（进度/摘要断言）+ tests/self-evolution.test.ts（新）、docs/DECISIONS.md（D-026）、docs/ARCHITECTURE.md、README.md、.agent 簿记
Forbidden scope: 任务/对话完成自动写 CURRENT_STATE.md（D-025 覆盖面保持）、规划门豁免 agent/*.md（用户拍板不豁免）、员工扩大自身权限（不可改 settings.json）
Dependencies: 用户批准的范围（A+C 派发进度 / 允许改 POLICIES / 规划门不豁免 / knowledge 一并提交）、TASK-023（OperationDto 底座）、TASK-028（单文件 git 提交 + ensureStateEditAllowed 模式）
Expected output: 派发时 CLI 打印 \r 进度行、Web 操作中心显示 summary；员工可在 developing 改四份自维护文档 + 知识，系统自动单文件提交（evolve: 前缀）
Outcome: 派发进度 + 自我进化全部落地。src/core/operation-manager.ts OperationDto/OperationEvent 增 summary?:string + execute emit 同步 dto.summary；factory-application dispatchItem 6 阶段 progress 事件 + dispatchPlan 波形 summary（N/M 完成 · 执行中 · 等待中）+ orchestrate onProgress + summaryOf/progressOf 纯函数；cli-program plan run/chief run 订阅 \r 进度行；web OperationsDrawer 展示 summary + styles .operation-summary；current-state.ts ensureAgentDocsAllowed 参数化放行 ROLE/GOALS/OPERATING_SYSTEM/POLICIES 四文档 + prepareRuntime 幂等补放行；ENTRY.md.tmpl 追加自我进化引导节；runAgent/runChat/runJob 后 commitSelfEvolution 单文件提交（evolve: 更新 <basename>）+ knowledgeWrite 提交（evolve: 更新知识），均 best-effort 不阻断；规划门脏审计不豁免 agent/*.md（planning 改 ROLE.md → item failed）。新增 tests/self-evolution.test.ts(+4)、orchestration +3。docs/DECISIONS.md D-026 ADR。注：D-026 派发进度半已于 TASK-030 移除（D-027），自我进化半保留。未 push。
Started at: 2026-08-05 18:00 +0800
Updated at: 2026-08-05 18:00 +0800
```
...
```text
Task ID: TASK-030
Title: 移除 Chief 编排 / Todo 状态机 / MCP 接入（D-027）
Owner agent: claude-20260803-01
Status: DONE
Branch/worktree: main
Allowed scope: 三块移除 + 直接依赖（见下）+ 共享基础设施保留（OperationManager/run/chat/job/knowledge/self-evolution/Web 操作中心原样保留）
Forbidden scope: 误删共享基础设施（OperationManager/OperationStore/OperationDto/OperationEvent、runAgent/runChat/runJob、commitSelfEvolution/prepareRuntime、knowledge/skills/backup/trash/prune/doctor、Web「任务」tab、AgentRole/role 字段）
Dependencies: 用户批准（TASK-030 计划：Chief+Todo 合并移除 / MCP 整块移除 / 派发进度反馈一并移除）
Expected output: 删除 Todo 状态机（task-schema.ts/task-store.ts）+ Chief 编排（factory-application 全部 plan/编排方法与私有辅助 + process-runner skipSelfEvolution）+ MCP 整块（mcp-server.ts + /mcp 路由 + enableMcp/mcpToken + @modelcontextprotocol/sdk 依赖）+ 派发进度反馈（OperationDto/OperationEvent.summary + CLI \r 进度 + web summary 展示）；runAgent commitSelfEvolution 无条件调用；Web 去 Todo/Chief 视图与 plan 方法；测试删除 4 文件 + 裁剪 4 文件；文档 D-027 ADR + 改写 D-026 + 标记 D-017/D-018/D-021/D-022/D-023/D-024 废弃
Outcome: 三块移除全部落地（详见 .agent/TASK_HANDOFF.md）。额外移除 /code-review 发现的 git.ts 死导出（gitDiff/snapshotWorkspaceHash + collectFiles/WorkspaceFile/isIgnoredTemp + path/createHash/fs-extra import + 3 测试用例）；web-server.test.ts 描述块改名「Web chat operation」。/code-review 双轴通过（Standards 0 硬违规 / Spec 0 缺陷）。全量单测 272 过 40 文件；tsc/lint 全绿；孤儿引用无残留。已 commit main（未 push）。
Acceptance criteria:
  - npx tsc --noEmit（tsconfig.json + tsconfig.web.json）+ npm run lint（含 prettier）全绿
  - 全量 npm test 从 343 降到约 272 全绿（40 文件）
  - 无孤儿引用：grep task-schema/task-store/McpBackend/createTaskPlan/runTaskPlan/summaryOf/progressOf/operationManager.summary/skipSelfEvolution/gitDiff/snapshotWorkspaceHash 无残留
...
Dependencies: TASK-042（D-041 P3，M3）
...
```text
Task ID: TASK-047
Title: identity_edits 生效（D-043）——direct 聊天直改 / proposal_required 提案门，identity-guard 硬门始终生效
Owner agent: claude-20260807-01
Status: DONE
Branch/worktree: main
Allowed scope: src/core/proposal-ledger.ts（appliedWithoutAnchor 第三参 identityEdits + IdentityEdits 类型 + readIdentityEdits + maybeEnforceIdentityProtocol 透传）、src/application/factory-application.ts（enforceIdentityProtocol 条件展开传 identity_edits）、src/core/doctor.ts（proposal-ledger 检查项 direct 分支）、src/schemas/agent-schema.ts（注释更新）、tests/proposal-ledger.test.ts、tests/self-evolution.test.ts、tests/doctor.test.ts、docs/DECISIONS.md、README.md、.agent 簿记
Forbidden scope: 不改 identity-guard 硬门本身（direct 只豁免提案门，红线词/岗位标题/宪法锚点保护不变）；不引入新字段/新配置项
Dependencies: 用户点名「identity_edits 声明未实现」（看板清点）、D-041 P1-4（字段声明与语义预留）
Expected output: identity_edits=direct（用户在聊天直接授权可直改核心身份）→ 对账跳过「未授权」判定（appliedWithoutAnchor 返回空、enforced 不拦截不写 CURRENT_STATE、doctor proposal-ledger pass + 标注 direct 模式）；proposal_required（默认）维持提案批准门；identity-guard 锚点硬门在 direct 下仍生效（整删红线词仍拒提交）
Acceptance criteria: 新增测试全绿（proposal-ledger：direct 整删红线词放行 / 缺省 proposal_required / readIdentityEdits 读回；self-evolution：settleActive 全链 direct 合规显著改动放行提交、proposal_required 同改动仍拦截、direct 下整删红线词仍被 identity-guard 硬门拦截；doctor：direct → pass + detail 标注）；npm test/build/lint/tsc 全绿；任务完成即 commit（不 push）
Started at: 2026-08-07 12:15 +0800
...
Dependencies: 用户拍板「0.5.9 可以升级到 0.7.0」「保持一个 profile = 一个飞书应用 = 一个员工」（本会话决策）
...
Dependencies: 用户拍板「KeepAlive + 状态同步」「走任务流程（推荐）」（本会话诊断：bridge 进程 SIGTERM 后 launchd 无 KeepAlive 不重启；reconcileServices 仅在 Web 控制台启动时运行，无 Web 场景永不恢复；CURRENT_STATE 显示运行中但服务实际已死）
