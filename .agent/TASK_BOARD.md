...
| TASK-021 | Chief/Todo/MCP 骨架（前端票：T01 git 基础 + T02 结构化 result + T03 取消单操作 + T04 Todo 领域核心 + T08 Chief 角色 + T11 MCP 传输+静态 bearer） | claude-20260803-01 | IN_PROGRESS | main                               | git.ts/usage.ts/operation-manager/task-schema/task-store/agent-schema/registry-schema/create-agent/cli-program/factory-application/templates/mcp-server/web-server 与测试 + DECISIONS ADR                                                                            | 用户批准的多 Agent 协作骨架 spec（.scratch/spec-chief-todo-mcp.md）与 13 票计划（.scratch/chief-todo-mcp/issues/） | 2026-08-05 11:30 +0800 |
