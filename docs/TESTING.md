...
- React 组件测试：首次初始化、员工创建、状态展示、Markdown 编辑、空状态，以及员工详情「Todo」标签（计划/任务项状态渲染、2 秒轮询刷新、计划级确认/驳回门、审查门确认合并/驳回返工）与「Chief 编排」流水线视图（阶段条 + 聚合进度渲染、展开审查结论与闸门、2 秒轮询、`role=chief` 才显示该标签）。
