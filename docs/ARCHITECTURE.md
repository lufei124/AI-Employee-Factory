...
- **Web 视图**：员工详情页「Todo 任务」标签（与「任务」= 定时 Job 区分）按 2 秒轮询展示计划与任务项状态；`draft` 计划提供确认/驳回门，`awaiting_review` 项渲染审查结论并提供确认合并/驳回返工。**Chief 编排视图**（仅 `role=chief` 员工显示）：把 Chief 拥有的每个目标（plan）渲染成一条流水线卡片——阶段条（拆解 → 计划确认 → 执行 → 审查 → 结果，按派生状态点亮）+ 聚合整体进度（如 `2/3 完成 · 1 待审查`），展开复用任务项渲染与两种闸门。add/派发仍由 CLI `plan`/`chief run` 完成，Web 只读 + 两种闸门（D-022）。
